...
const EventNodeInternal = ({ data, selected }: { data: EventNodeData; selected: boolean }) => {
  ...
  return (
    <div
      ...
      style={
        data.isWarning && !data.isBlinking
    ...
    >
      ...
      <div className="node-body">
        ...
        Object.keys(data.originalData[data.type].params.data).length > 0 ? (
          <div
            className="node-params-expanded"
            style={{
              marginTop: '8px',
          ...
          >
            ...
            {Object.entries(data.originalData[data.type].params.data).map(
              ([key, valObj]: [string, any]) => {
                const paramType = Object.keys(valObj)[0];
                const val = valObj[paramType];
                const isMessageId =
                  key.toLowerCase() === 'messageid' ||
                  key.toLowerCase() === 'message_id' ||
                  key.toLowerCase() === 'msgid';
                const resolvedText = isMessageId
                  ? resolveMessageText(val, data.messageDict)
                  : undefined;
                ...
                return (
                  <div key={key} style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                    ...
                    >
                      <span
                        style={{
                          color: isMessageId ? '#38bdf8' : '#94a3b8',
                          fontWeight: isMessageId ? 600 : 400,
                          flexShrink: 0,
                        }}
                      >
                        {key}
          ...
          <div className="node-param">
            ...
            <span className="node-param-val">
              {data.originalData[data.type]?.params?.data
                ? Object.keys(data.originalData[data.type].params.data).length
                : 0}
            </span>
          </div>
        )}
...
export const EntryPointNode = React.memo(EntryPointNodeInternal, (prev, next) => {
  return prev.data.name === next.data.name && prev.data.isBlinking === next.data.isBlinking;
});
