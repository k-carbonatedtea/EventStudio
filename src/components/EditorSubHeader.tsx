...
export default function EditorSubHeader({
  ...
  const currentEventName =
    ...
    evflData?.name ||
    (filePath ? filePath.split(/[/\\]/).pop()?.replace(/\.bfevfl$/i, "") : "") ||
  ...
  const handleConfirmName = async () => {
    ...
    if (trimmed !== currentEventName) {
      ...
      showToast(locale === 'zh' ? `已重命名事件流为: ${trimmed}` : `Renamed flowchart to: ${trimmed}`, "success");
    }
    setIsEditingName(false);
  };
              ...
              <div className="timeline-dropdown-list">
                {history.map((item, idx) => {
                  ...
                  if (idx === 0 || title === "初始文件状态" || title === "Initial File State") {
                    ...
                    const nodeType = title.replace(/^(创建|Create\s+)/, "").replace(/\s*(节点)?$/, "");
                  ...
                  } else if (title.startsWith("删除节点连线") || title.startsWith("Delete connection line")) {
                    title = t("timeline.deleteEdges");
                  } else if (title.startsWith("重命名入口点:") || title.startsWith("Rename entry point:")) {
                  ...
                  } else if (title.startsWith("重命名流程图:") || title.startsWith("Rename flowchart:")) {
                  ...
                  if (detail.endsWith("加载完成") || detail.endsWith("loaded successfully") || detail.endsWith("Loaded")) {
                    ...
                    detail = t("timeline.deleteEdgesDetail", { count: countMatch ? countMatch[0] : "1" });
                  ...
                  } else if (detail === "同步内部 Flowchart 标识" || detail === "Synchronize flowchart identifier") {
                    detail = t("timeline.syncFlowchartDetail");
                  } else if (!detail) {
                    detail = idx === 0 ? t("timeline.loadedDetail", { name: "File" }) : t("timeline.updateNodeDetail", { type: "Node" });
                  }
