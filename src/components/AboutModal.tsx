  ...
  const rustLibraries = [
    {
      ...
      desc:
        locale === 'zh'
          ? '高性能轻量级桌面端应用程序框架'
          : 'High-performance desktop application framework',
      author: 'Tauri Apps',
    },
    {
      name: 'revfl',
      desc:
        locale === 'zh'
          ? '任天堂 EVFL (EventFlow) 二进制与 JSON 序列化库'
          : 'Nintendo EVFL binary & JSON serialization library',
      author: 'k-carbonatedtea / leoetlino',
    },
    {
      name: 'roead',
      desc:
        locale === 'zh'
          ? 'SARC 归档包、Yaz0 压缩及 BYML/AAMP 数据解析库'
          : 'SARC, Yaz0 compression & BYML/AAMP format library',
      author: 'NiceneNerd',
    },
    {
      name: 'msyt / msbt',
      desc:
        locale === 'zh'
          ? '任天堂 MSBT 消息文本格式与 MSYT 解析转换库'
          : 'Nintendo MSBT text format and MSYT parser',
      author: 'NiceneNerd',
    },
  ];
  ...
  const frontendLibraries = [
    {
      ...
      desc:
        locale === 'zh'
          ? '现代化交互式节点流向图可视化引擎'
          : 'Modern interactive node graph visualization engine',
      author: 'xyflow',
    },
    {
      ...
      desc:
        locale === 'zh'
          ? '基于 VS Code 的代码与文本高亮编辑器'
          : 'VS Code-based code and text editor component',
      author: 'suren-atoyan',
    },
    {
      name: 'dagre',
      desc:
        locale === 'zh'
          ? '有向图层次化自动排版与布局计算算法'
          : 'Directed graph layered layout calculation engine',
      author: 'dagrejs',
    },
    {
      name: 'lucide-react',
      desc: locale === 'zh' ? '简洁美观的现代化矢量图标库' : 'Clean & modern SVG icon library',
      author: 'Lucide',
    },
    {
      name: 'js-yaml',
      desc:
        locale === 'zh' ? '标准 YAML 文件解析与序列化工具' : 'Standard YAML parser and serializer',
      author: 'nodeca',
    },
  ];
      ...
      >
        ...
        <div className="modal-body" style={{ overflowY: 'auto', maxHeight: '55vh', padding: 20 }}>
          {activeTab === 'info' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ textAlign: 'center', padding: '10px 0' }}>
                <h2 style={{ margin: '0 0 6px 0', fontSize: '1.4rem', color: '#38bdf8' }}>
                  {t('about.title')}
                </h2>
                <div style={{ color: '#94a3b8', fontSize: '0.9rem' }}>{t('about.version')}</div>
              </div>

              <div className="settings-tip">{t('about.desc')}</div>
              ...
              >
                <h4 style={{ margin: '0 0 10px 0', color: '#38bdf8', fontSize: '0.92rem' }}>
                  {t('about.devContactTitle')}
                </h4>
                <div
                  style={{
                    display: 'flex',
          ...
          {activeTab === 'libraries' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <h4 style={{ margin: '0 0 8px 0', color: '#38bdf8', fontSize: '0.92rem' }}>
                  {t('about.rustLibsTitle')}
                </h4>
          ...
          {activeTab === 'credits' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div className="settings-tip">{t('about.creditsTip')}</div>
              ...
              >
                ...
                >
                  <li>
                    <strong>k-carbonatedtea / leoetlino</strong> - revfl parser and EVFL binary
                    format reversing.
                  </li>
                  <li>
                    <strong>NiceneNerd</strong> - roead (SARC, Yaz0, BYML, AAMP), msyt.
                  </li>
                  <li>
                    <strong>leoetlino</strong> - BotW reverse engineering, BCML / SARC tooling.
                  </li>
                </ul>
              </div>
