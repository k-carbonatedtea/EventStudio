...
export default function SwitchEditorModal({ node, events, actors, onSave, onCancel }: SwitchEditorModalProps) {
  ...
  const handleCaseChange = (id: string, field: 'value' | 'targetEvent', newValue: string | number) => {
            ...
            <table className="params-table cases-table">
              <thead>
                <tr>
                  ...
                  <th style={{ width: '50%' }}>{locale === 'zh' ? '目标事件 ID' : 'Target Event ID'}</th>
                ...
                {casesList.length === 0 ? (
                  <tr>
                    <td colSpan={3} style={{ textAlign: 'center', padding: '20px', color: '#94a3b8' }}>
                      {locale === 'zh' ? '暂无条件分支，请点击下方按钮添加。' : 'No cases defined. Add a case below.'}
                    </td>
                  </tr>
                ) : (
                  casesList.map((c) => (
                    <tr key={c.id}>
                      ...
                      <td>
                        <select 
                          ...
                          value={c.targetEvent}
                          onChange={(e) => handleCaseChange(c.id, 'targetEvent', parseInt(e.target.value, 10))}
                        >
